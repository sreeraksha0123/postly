...
try {
  ...
  const getSession = async (chatId) => {
    ...
    if (s) {
      await redis.expire(`bot:session:${chatId}`, 1800)
      return JSON.parse(s)
    }
    return null
  }
  const saveSession = async (chatId, session) => {
      await redis.set(`bot:session:${chatId}`, JSON.stringify(session), 'EX', 1800)
  }
  ...
  // Create a dummy bot object so imports don't fail
  bot = {
    api: {
      setWebhook: async () => console.warn('[Telegram] Bot not initialized'),
      getWebhookInfo: async () => ({})
    },
    start: async () => console.warn('[Telegram] Bot not initialized'),
    on: () => {},
    command: () => {},
    callbackQuery: () => {}
  }
